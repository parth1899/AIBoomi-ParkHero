...
def set_coordinates(apps, schema_editor):
    ...
    for name, lat, lon in coords:
        # Case-insensitive match to reduce risk of missing entries due to capitalization
        Facility.objects.filter(name__iexact=name).update(latitute=lat, longitude=lon)
...
def unset_coordinates(apps, schema_editor):
    ...
    for name in names:
        Facility.objects.filter(name__iexact=name).update(latitute=None, longitude=None)


class Migration(migrations.Migration):

    dependencies = [
        ('atlas', '0007_update_facility_coordinates'),
    ]
